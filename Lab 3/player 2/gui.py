...
def on_message(client, userdata, message):
  if(message.topic == "ece180d/zeid_rps"):
    ...
    if pick_number != -1:
        win_lose_label.config(text = "Pick your move")
...
def spin():
    ...
    client.publish("ece180d/warren_rps", user_choice_value, qos=1)
    ...
    if user_choice_value == 0: # rock
        ...
        if pick_number == 0: # rock
            win_lose_label.config(text = "It's a tie! Waiting on the other player.")
        elif pick_number == 1: # paper
            win_lose_label.config(text = "Paper beats Rock. You lose! Waiting on the other player.")
        elif pick_number == 2: # scissors
            win_lose_label.config(text = "Rock beats Scissors. You win! Waiting on the other player.")

    elif user_choice_value == 1: # paper

        if pick_number == 0: #rock
            win_lose_label.config(text = "Paper beats Rock. You win! Waiting on the other player.")
        elif pick_number == 1: # paper
            win_lose_label.config(text = "It's a tie! Waiting on the other player.")
        elif pick_number == 2: # scissors
            win_lose_label.config(text = "Scissors beats Paper. You lose! Waiting on the other player.")

    elif user_choice_value == 2: # scissors

        if pick_number == 0: # rock
            win_lose_label.config(text = "Rock beats Scissors. You lose! Waiting on the other player.")
        elif pick_number == 1: # paper
            win_lose_label.config(text = "Scissors beats Paper. You win! Waiting on the other player.")
        elif pick_number == 2: # scissors
            win_lose_label.config(text = "It's a tie! Waiting on the other player.")
...
# label win lose
win_lose_label = Label(root, text = "Waiting on other player", font = ("Helvetica", 18), bg = "white")
